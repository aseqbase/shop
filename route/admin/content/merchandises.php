...
(new MiMFa\Library\Router())
    ->Get(function () use ($module) {
        module("PrePage");
        $pp = new PrePage();
        $pp->Title = "Merchandises Management";
        $pp->Image = "box";
        ...
        $styles = Html::Style(".{$module->Name} tr td input{
        ...
        $module->CellsValues = [
            ...
            "Count" => function ($v, $k, $r) {
                return $v . ($r["CountUnit"]??\_::$Config->CountUnit);
            },
            "Price" => function ($v, $k, $r) {
                return $v . ($r["PriceUnit"]??\_::$Config->PriceUnit);
            },
        ...
        view("main", [
            "Content" => $pp->Handle() . $styles . $module->Handle()
        ]);
    })
    ->Put(function () {
        ...
        if ($MerchandiseId = $received["MerchandiseId"])
            try {
                table("Merchandise")->Update("`Id`=:Id", [
                    ...
                    "UpdateTime" => \_::$Config->CurrentDateTime
                ]);
