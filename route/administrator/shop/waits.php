...
$routeHandler = function ($data) {
    ...
    $module->SelectQuery = table("Shop_Request")->As("R")
        ...
        U.Name AS 'User', U.Signature AS 'UserPath', R.Collection AS 'Collection',
        R.Count AS 'Count', M.Unit AS 'Unit',
        R.Amount AS 'Amount'", "R.Count>0");
    $module->KeyColumns = ["Item", "User"];
    $module->IncludeColumns = ["User", "Item", "Count", "Amount", "Collection", 'Access', 'UpdateTime'];
    $module->FilterColumns = ["Collection"];
    ...
    $module->CellsValues = [
        ...
        "Collection" => function ($v, $k, $r) {
            return $v?Struct::Link($v, \_::$Address->UrlBase."?$k=".urlencode($v)):Struct::Span("Not Collected", null, ["class"=>"not-collected"]);
        }
    ];
    ...
    pod($module, $data);
    return Struct::Style("
        .{$module->MainClass} tr:has(* .not-collected){
            opacity: 0.7;
        }
        .{$module->MainClass} tr .not-collected{
